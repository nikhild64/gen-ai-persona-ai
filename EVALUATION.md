...
2. **DevTools → Application → Session Storage:** paste a Gemini key via
   the Settings modal (`⚙` icon). Verify **no** `byo-key:*` entry appears
   (keys are in-memory for the page load only per AD-11). Confirm chat
   works after save — proves BYO-Key without persistent browser storage.
