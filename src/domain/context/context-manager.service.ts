...
export class ContextManager {
  ...
  private readonly summaryChains = new Map<StorageKey, Promise<void>>();

  async onTurnComplete(threadKey: StorageKey): Promise<void> {
    if (!FEATURE_ROLLING_SUMMARY) return;

    const prev = this.summaryChains.get(threadKey) ?? Promise.resolve();
    let release!: () => void;
    const gate = new Promise<void>((resolve) => {
      release = resolve;
    });
    const chained = prev.then(() => gate);
    this.summaryChains.set(threadKey, chained);
    await prev;
    try {
      await this.runSummary(threadKey);
    } finally {
      release();
    }
  }

  private async runSummary(threadKey: StorageKey): Promise<void> {
