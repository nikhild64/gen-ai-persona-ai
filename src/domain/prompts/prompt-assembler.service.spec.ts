...
describe('PromptAssembler solo mode', () => {
  ...
  it('injects rolling summary in system and recent turns as native history', () => {
    ...
    expect(history[0]?.content).toContain('m-13');
    expect(history[history.length - 1]?.content).toContain('current-q');
    expect(history.some((m) => m.content.includes('m-12'))).toBe(false);
  });
...
describe('PromptAssembler ask-both-blended mode (post-sprint)', () => {
  ...
  it('AC-5: keep-going path appends a synthetic user turn after native history', () => {
    ...
    const out = assembler.compose('musk', thread, 'ask-both-blended');
    expect(out.messages).toHaveLength(4);
    expect(out.messages[1]?.content).toContain('original question');
    expect(out.messages[2]?.content).toBe('first blended reply');
    const userMsg = out.messages[3]?.content ?? '';
    expect(userMsg).toContain('Continue this Blended discussion');
    expect(userMsg).toContain('fresh angle');
    const system = out.messages[0]?.content ?? '';
    expect(system).not.toContain('first blended reply');
  });
  ...
  it('composeFromEntry uses voiceReminder from custom entry', () => {
    const thread = threadFrom([msg('user', 'hello', 1)]);
    const out = assembler.composeFromEntry({
      prompt: {
        identityBlock: 'Custom identity',
        voiceRules: 'Custom voice',
        refusalRules: 'Custom refusal',
        fewShots: [],
        askBothCollabExamples: [],
        driftRefresh: 'Custom drift',
        selfVerificationChecklist: 'Custom check',
        capRefusalTemplate: 'cap',
        quotaExhaustedTemplate: 'quota',
        offDomainTemplate: 'off',
        politicalTemplate: 'pol',
        adultTemplate: 'adult',
        promptInjectionTemplate: 'inj',
        fabricationBaitTemplate: 'fab',
        hostileUserTemplate: 'host',
        modelFailureTemplate: 'fail',
        selfIdentificationResponse: 'id',
      },
      greeting: 'Hi',
      inputPlaceholder: 'Ask',
      starterQuestions: [],
      providerId: 'gemini',
      fullDisplayName: 'Custom',
      tagline: 'Tag',
      era: 'Living',
      disclaimerTier: 'contemporary',
      voiceReminder: 'CUSTOM_VOICE_REMINDER_XYZ',
    }, thread);
    expect(out.messages[0]?.content).toContain('CUSTOM_VOICE_REMINDER_XYZ');
  });
});
